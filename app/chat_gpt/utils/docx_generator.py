...
async def test_kp_generation():
    ...
    Функционал: выбор ресторанов, меню, корзина, онлайн-оплата, отслеживание заказа.
    Технологии: React Native для фронтенда, Node.js для бэкенда, MongoDB для базы данных.
    Дополнительно: система рейтингов, push-уведомления, геолокация.
    Бюджет: около 150000 рублей, срок: 2-3 месяца.
    """

    kp_service = KPService()

    try:
        print("🚀 Начинаем генерацию КП...")
        project_name = "Приложение доставки еды"

        docx_filepath = await kp_service.create_kp_document(project_description, project_name)

        print(f"✅ КП успешно создано: {docx_filepath}")
        print(f"📁 Полный путь: {os.path.abspath(docx_filepath)}")

        # Показываем содержимое сгенерированного файла
        with open(docx_filepath.replace('.docx', '.md'), 'r', encoding='utf-8') as f:
            content = f.read()
            print("\n📋 Содержание КП (Markdown):")
...
async def generate_kp_for_project(project_description: str, project_name: str) -> str:
    ...
    Args:
        project_description: Описание проекта от клиента
